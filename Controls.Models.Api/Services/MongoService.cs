...
public class MongoService : IMongoService
{
    ...
    public MongoService(string url, string database) : this(MongoClientSettings.FromConnectionString(url), database)
    {
    }

    private MongoService(MongoClientSettings settings, string database)
    {
    ...
    /// <returns></returns>
    public static MongoService GetInstance(string url, string database)
    {
    ...
    /// <param name="database">数据库</param>
    /// <returns></returns>
    public static MongoService ThreadLocalInstance(string url, string database)
    {
        MongoService instance;
        if (CallContext<MongoService>.GetData(url + database) == null)
        {
            instance = new MongoService(url, database);
            CallContext<MongoService>.SetData(url + database, instance);
        }

        instance = CallContext<MongoService>.GetData(url + database);
        return instance;
    }
    ...
    /// <param name="upsert">如果它不存在是否插入文档</param>
    /// <returns></returns>
    public UpdateResult UpdateOneByO<T>(string collection, Expression<Func<T, bool>> filter, UpdateDefinition<T> update, bool upsert)
    {
        return Database.GetCollection<T>(collection).UpdateOne(filter, update, new UpdateOptions()
        {
            IsUpsert = upsert
    ...
    /// <returns></returns>
    public T UpdateOne<T>(string collName, Func<Codess, bool> value, Expression<Func<T, bool>> filter, UpdateDefinition<T> update)
    {
